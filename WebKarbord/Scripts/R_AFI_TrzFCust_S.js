var ViewModel = function () {
    ...
    $('#SaveColumns').click(function () {
        SaveColumn(ace, sal, group, rprtId, "/ReportAFI/TrzFCust_S", columns, self.SettingColumnList());
    });
    ...
    self.ShowFDocR_S = function (Band) {
        if (Band.CustCode == "") {
            return showNotification("به دلیل ثبت نشدن خریدار امکان نمایش وجود ندارد", 0)
        }
    ...
    function CreateTableReport(data) {
        ...
        $('#TableReport').append(
            ...
            '        <img src="/Content/img/view.svg" width="18" height="18" style="margin-left:10px" />' + translate('ریز گردش اسناد فروش') +
            '    </a >' +
            ' </td >' +
            '</tr>' +
            '</tbody>' +
    ...
    self.ShowPrintForms = function (item) {
        ...
        setReport(self.TrzFCust_SList(), data, printVariable);
    };
    ...
    $('#Print').click(function () {
        ...
        printName = null;
        sessionStorage.ModePrint = "ReportTrzFCust_S";
        GetPrintForms(sessionStorage.ModePrint);
    ...
    self.PageIndexTrzFCust_S = function (item) {
        return CountPage(self.filterTrzFCust_SList(), self.pageSizeTrzFCust_S(), item);
    };
